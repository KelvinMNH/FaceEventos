...
function TotemSaida() {
            ...
            try {
                ...
                if (data) {
                    ...
                    if (!data.habilitar_checkout) {
                        setMensagem('Este evento não possui checkout habilitado');
                        setStatus('erro');
                    }
                } else {
    ...
    const handleBiometriaCheckout = async (simularSucesso = true) => {
        console.log('🔍 Iniciando checkout. Sucesso?', simularSucesso);
                        ...
                        if (participantesAtivos.length > 0) {
                            ...
                            try {
                                ...
                                if (dataSaida.success) {
                                    ...
                                    setMensagem(`Até logo, Dr(a). ${participanteAleatorio.nome}!`);
                                ...
                                } else if (dataSaida.already_checked_out) {
                                    setStatus('ja_saiu');
                                    setParticipante(participanteAleatorio);
                                    setMensagem('Checkout já realizado anteriormente');
                                    ...
                                    setStatus('erro');
                                    setMensagem(dataSaida.msg || 'Erro ao registrar saída');
                                    setTimeout(() => {
                                        setStatus('aguardando');
                                        setMensagem('');
                                    }, 3000);
        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {status === 'aguardando' && (
                            <>
                                ...
                                <p style={{ color: '#666', fontSize: '1.1rem' }}>
                                    Posicione seu dedo no leitor para registrar sua saída
                                </p>
                            </>
                        )}
                        ...
                        {status === 'sucesso' && participante && (
                            <>
                                ...
                                <p style={{ fontSize: '1.1rem', color: '#198754', fontWeight: '600', marginTop: '1rem' }}>
                                    Esperamos vê-lo(a) novamente em breve!
                                </p>
                            </>
                        )}

                        {status === 'ja_saiu' && participante && (
                            <>
                                <div style={{
                                    width: '150px',
                                    height: '150px',
                                ...
                                <h2 style={{ fontSize: '1.8rem', color: '#ff6600', marginBottom: '1rem' }}>
                                    Checkout Já Realizado
                                </h2>
                                <p style={{ fontSize: '1.2rem', color: '#333', marginBottom: '0.5rem' }}>
                                    Dr(a). {participante.nome}
                                </p>
                                <p style={{ fontSize: '1.1rem', color: '#666' }}>
                                    Sua saída já foi registrada
                                </p>
                            </>
                        )}
                    ...
                    {/* Botões de Teste */}
                    {status === 'aguardando' && evento && evento.habilitar_checkout && (
                        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center' }}>
                            <button
                                onClick={() => handleBiometriaCheckout(true)}
                                style={{
                                    background: 'linear-gradient(135deg, #198754 0%, #20c997 100%)',
                                    color: 'white',
                                    border: 'none',
                            ...
                            <button
                                onClick={() => handleBiometriaCheckout(false)}
                                style={{
